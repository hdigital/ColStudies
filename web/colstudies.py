...
def home():
    ...
    wp.add(
        jp.P(
            text="- Studies will show all studies of the Colectica Repository. You can then select a study to perform actions."
        )
    )
    wp.add(
        jp.P(
            text="- Series will show all series of the Colectica Repository. You can then show all studies within a series."
        )
    )
    wp.add(
        jp.P(
            text="- Actions include registration of DOIs with da|ra and managing file download permissions with DBKEdit."
        )
    )
    jp.Br(a=wp)
