...
def get_reference(ddixml, itemxpath):
    ...
    )  # no xml: all namespaces were removed
...
def get_referenceFromAttributePair(ddixml, itemxpath):
    ...
    # Get properties
    urnliststring = getValueFromXPathAttributePair(
        root, itemxpath + "/UserAttributePair", "extension:InstrumentReferences"
    )
    if urnliststring is None:
        return ref
    urnlist = ast.literal_eval(urnliststring)
    if urnlist is None:
        return ref
    if urnlist[0] is None:
        return ref

    urnparts = urnlist[0].split(":")

    if urnparts[2] is None:
        return ref
    if urnparts[3] is None:
        return ref
    if urnparts[4] is None:
        return ref
...
def getValueFromSubPathWithXPathWithAttribute(
    node, xpath, subpath, attributename, attributevalue
):
    ...
    try:
        ...
        for uid in userids_all:
            ...
            if uid.find("./{ddi:reusable:3_3}AttributeKey") != None:
                ...
                if (
                    uid.find("./{ddi:reusable:3_3}AttributeKey").text
        ...
        if not found:
            ...
            if parent != None:
                ...
                jsonString = (
                    '{"Title":{"en":"Crosstabs"},"Description":{},"ValueType":0,"HasValue":true,"RelationshipTargetType":"00000000-0000-0000-0000-000000000000","StringValue":"'
                    + annotationtext
                    + '","MultilingualStringValue":{},"BooleanValue":false,"HasDefinedType":false,"DisplayLabel":"Crosstabs"}'
                )
                # parent.add
...
def getStudyNo(ddixml):
    ...
    userids_all = root.findall(
        './/StudyUnit/UserID[@typeOfUserID="GESIS Study Number"]'
    )
    for uid in userids_all:
        StudyNo = uid.text
...
def buildCV(ddixml, xpath, Limit):
    ...
    if not cvxml == "":
        ...
        codes = root.findall(
            ".//Code"
        )  # allow any level here, do not use './/CodeList/Code'
        for code in codes:
        ...
        for ref in reflist:
            ...
            if i >= Limit:  # to avoid key error later
                ...
                if cvlevel1xml is not None:
                    ...
                    # print(cvlevel1xml)
                    texten = root.find(
                        ".//Category/Label/Content[@" + xml + 'lang="en"]'
                    ).text
                    textde = root.find(
                        ".//Category/Label/Content[@" + xml + 'lang="de"]'
                    ).text
